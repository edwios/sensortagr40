...
void ble_ap3216c_on_ble_evt(ble_envsense_t * p_ap3216c, ble_evt_t * p_ble_evt)
{
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            p_ap3216c->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
            break;
...
//    NRF_LOG_DEBUG("ble_char_accel_add: add gatt char"); NRF_LOG_FLUSH();
    err_code = sd_ble_gatts_characteristic_add(p_ap3216c->service_handle,
                                       &char_md,
                                       &attr_char_value,
                                       &p_ap3216c->visible_char_handles);
...
void ble_ap3216c_update(ble_envsense_t *p_ap3216c, ap3216c_ambient_values_t * ap3216c_ambient_values)
{
    ...
    if (p_ap3216c->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        uint16_t visible_value = ap3216c_ambient_values->ambient_visible_value;
        uint16_t ir_value = ap3216c_ambient_values->ambient_ir_value;
        uint16_t lux_value = ap3216c_ambient_values->ambient_lux_value;

        hvx_params.handle = p_ap3216c->visible_char_handles.value_handle;
        ...
        memset(&hvx_params, 0, sizeof(hvx_params));
        hvx_params.handle = p_ap3216c->ir_char_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = 0;
        hvx_params.p_len  = &len;
        ...
        memset(&hvx_params, 0, sizeof(hvx_params));
        hvx_params.handle = p_ap3216c->lux_char_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = 0;
        hvx_params.p_len  = &len;
...
void ble_envsense_temperature_update(ble_envsense_t *p_ap3216c, temp_value_t * temperature_value)
{
    ...
    if (p_ap3216c->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        memset(&hvx_params, 0, sizeof(hvx_params));
        hvx_params.handle = p_ap3216c->temp_char_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = 0;
        hvx_params.p_len  = &len;
