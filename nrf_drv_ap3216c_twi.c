...
uint8_t twi_tx_buffer[AP3216C_TWI_BUFFER_SIZE];


static void nrf_drv_ap3216c_twi_event_handler(nrf_drv_twi_evt_t const * p_event, void * p_context)
{
    switch(p_event->type)
    {
...
uint32_t nrf_drv_ap3216c_init(void)
{
    ...
    const nrf_drv_twi_config_t twi_ap3216c_config = {
    ...
    err_code = nrf_drv_twi_init(&m_twi_instance, &twi_ap3216c_config, nrf_drv_ap3216c_twi_event_handler, NULL);
...
uint32_t nrf_drv_ap3216c_write_single_register(uint8_t reg, uint8_t data)
{
    ...
    err_code = nrf_drv_twi_tx(&m_twi_instance, AP3216C_ADDRESS, packet, 2, false);
...
uint32_t nrf_drv_ap3216c_read_registers(uint8_t reg, uint8_t * p_data, uint32_t length)
{
    ...
    err_code = nrf_drv_twi_rx(&m_twi_instance, AP3216C_ADDRESS, p_data, length);
    if(err_code != NRF_SUCCESS) return err_code;

    timeout = AP3216C_TWI_TIMEOUT;
